				...
				if ($product_fourn->find_min_price_product_fournisseur($obj->rowid) > 0) {
					if ($product_fourn->product_fourn_price_id > 0) {
						if ((!empty($conf->fournisseur->enabled) && !empty($user->rights->fournisseur->lire) && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD)) || (!empty($conf->supplier_order->enabled) && !empty($user->rights->supplier_order->lire)) || (!empty($conf->supplier_invoice->enabled) && !empty($user->rights->supplier_invoice->lire))) {
							...
							$pricecal=round($product_fourn->fourn_unitprice+(($product_fourn->fourn_tva_tx*$product_fourn->fourn_unitprice)/100));
							print '<span class="amount">'.$form->textwithpicto(price($product_fourn->fourn_price * (1 - $product_fourn->fourn_remise_percent / 100) - $product_fourn->fourn_remise).' '.$langs->trans("TTC"), $htmltext).'</span>';
						} else {
							$pricecal=round($product_fourn->fourn_unitprice+(($product_fourn->fourn_tva_tx*$product_fourn->fourn_unitprice)/100));
							print '<span class="amount">'.price($product_fourn->fourn_price).' '.$langs->trans("TTC").'</span>';
						}
					}
				}
			}
